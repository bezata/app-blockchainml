...
df = pl.read_parquet('hf://datasets/${dataset.id}/' + splits['${selectedSplit}'])`;
  ...
  return (
    <div className="p-4 md:p-6 bg-gray-900 min-h-screen">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <Card className="bg-gray-800 border-gray-700">
          ...
          <CardContent className="text-gray-300">
            {dataset.cardData.dataset_info?.map((info) => (
              <div key={info.config_name} className="mb-4 last:mb-0">
                ...
                <div className="space-y-1">
                  <InfoItem
                    label="Total Size"
                    value={`${(info.dataset_size / 1024 / 1024).toFixed(2)} MB`}
                  />
                  <InfoItem
                    label="Download Size"
                    value={`${(info.download_size / 1024 / 1024).toFixed(
                      2
                    )} MB`}
                  />
                  <InfoItem
                    label="Number of Splits"
                    value={info.splits.length}
                  />
                  {info.splits?.map((split: Split) => (
                    <InfoItem
                      key={split.name}
                      label={split.name}
                      value={`${split.num_examples.toLocaleString()} examples`}
                    />
                  ))}
                </div>
              </div>
      ...
      <Card className="bg-gray-800 border-gray-700 mb-6">
        <CardHeader>
          <CardTitle className="text-white">Use Dataset</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex flex-wrap gap-4">
            ...
            {(selectedFormat === "parquet" ||
              selectedFormat === "csv" ||
              selectedFormat === "pandas" ||
              selectedFormat === "polars") && (
              <>
                <Select
